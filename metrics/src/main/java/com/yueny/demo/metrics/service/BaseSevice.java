package com.yueny.demo.metrics.service;
...
public abstract class BaseSevice implements IBiz {
	 ...
	 * @return 映射后的目标数据列表
	 */
	protected final <T extends IBo, S extends IEntry> List<T> map(final List<S> sourceList, final Class<T> targetInfo) {
	 ...
	 * @return 映射后的业务对象
	 */
	protected final <T extends IBo, S extends IEntry> T map(final S sourceEntry, final Class<T> targetBOClass) {
	 ...
	 * @return 映射后的实体对象
	 */
	protected final <T extends IBo, S extends IEntry> S map(final T sourceBO, final Class<S> targetEntryClass) {

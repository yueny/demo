...
import com.yueny.demo.downlatch.r.PurchasedFailTransRecoverRunner;
...
public class CountDownLatchDemo {
	...
	private <T> List<List<T>> partition(final List<T> baseList,
			final Integer partitionSize) {
		Assert.notEmpty(baseList);
		Assert.isTrue(partitionSize != null && partitionSize > 0,
				"partionLimit is illegal.");
		final int totalCount = baseList.size();
		final int partitionCount = totalCount % partitionSize > 0 ? totalCount
				/ partitionSize + 1 : totalCount / partitionSize;
		final List<List<T>> partitionList = new ArrayList<List<T>>();
		for (int index = 1; index <= partitionCount; index++) {
			if (index < partitionCount) {
				final List<T> subList = baseList.subList((index - 1)
						* partitionSize, partitionSize * index);
	 ...
	 * @param processPhrase
	 */
	private void printResults(final TransResultHolder resultHolder,
		...
		for (int i = 0; i < fail.size(); i++) {
			logger.error("trans_id：{},{}failure, reason：{}", fail.get(i)
					.getTransId(), message, fail.get(i).getReason());
		}
		logger.info("{}处理总数:{}, 成功:{}, 失败：{}, 耗时:{}", message,
				resultHolder.getRecoverCount(), succ.size(), fail.size(), time);
	}
